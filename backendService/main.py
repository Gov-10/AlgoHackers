...
def get_time_index(year, month, day, hour):
    target = np.datetime64(pd.Timestamp(year=year, month=month, day=day, hour=hour))
...
def metadata():
    ...
    years = pd.to_datetime(ds.time.values).year.unique().tolist()
